...
ui <- shinyUI(fluidPage(
  ...
  # Application title
  titlePanel("Relationship of Population, Forest Area and Greenhouse gas emissions"),
...
server <- shinyServer(function(input, output) {
  ...
  output$myPlot <- renderPlot({
    ...
    g <- ggplot(WDIData.Population.GreenhouseX, aes(x = x, y = y, size = z, colour = as.factor(WDIData.Population.GreenhouseX$YEAR), label = YEAR)) + geom_point(alpha = 0.9)
